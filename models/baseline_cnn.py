...
class CNN(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)
        x1, word_weights = self.conv_and_pool(x, self.conv13)
        x2, word_weights = self.conv_and_pool(x, self.conv14)
        x3, word_weights = self.conv_and_pool(x, self.conv15)
        x = self.tanh(torch.cat((x1, x2, x3), 1))
        return x, word_weights
...
class Net(nn.Module):
    def __init__(self, weights_matrix, char_len , utter_len, predicate_num, kernel_num, k):
        ...
        self.fc2 = nn.Linear(3 * k * kernel_num, predicate_num)
    ...
    def forward(self, x):
        ...
        x = x.contiguous().view(-1, 3 * self.k * self.kernel_num)
        # 2 do feed forward
        out = self.dropout(x)
        out = self.fc2(out)
...
class CNNbaseline(MODEL_BASE):
    ...
    def __init__(self, *args, **kwargs):
        super(CNNbaseline, self).__init__(*args, **kwargs)
    ...
    def train(self, train_dir, kk = 0, folds = 10000, grid = False, grid_file = None):
        ...
        mux_stream_train = pescador.StochasticMux(streams_train, n_active=len(streams_train), rate=None, mode="with_replacement", random_state=33)
        ...
        for epoch in range(p["num_epochs"]):
            ...
            for i, (word_idx, label, _, _) in enumerate(mux_stream_train):
                ...
                if i % p["batch_size"] == 0 and i != 0:
                    answers = autograd.Variable(torch.LongTensor(labels)).view(p["batch_size"], -1).expand(p["batch_size"], p["char_len"]).contiguous().view(-1)
            ...
            if grid and (epoch + 1) % 10 == 0:
                ...
                streams_test = [pescador.Streamer(indexes_gen, ff, 1, p["utter_len"], p["char_len"], k=kk, mode="test", folds=folds) for ff
                                in train_files]
                mux_stream_test = pescador.StochasticMux(streams_test, n_active=len(streams_train), rate=None,
                                                         mode="exhaustive", random_state=33)
    ...
    def validate(self, test_file):
        ...
        with open(p["results_file"], "w") as results_file:
            for word_idxs, labels, characters, sentences in streamer:
                ...
                output = output.contiguous().view(p["char_len"], -1)
                entry = output.cpu().data.numpy()
                ent_array = []
                for ent in entry:
                    ent_array.append(sorted(enumerate(np.exp(ent)), key=lambda x: x[1], reverse=True)[0][0])
                ent_dict = dict(Counter(ent_array))
                ent_array = [ent_dict[x] / p["char_len"] if x in ent_dict else 0 for x in range(p["predicate_num"])]
                results_file.write(str(characters[0]) + "\t" + str(labels[0]) + '\t' + '\t'.join([str(y) for y in
                                                            sorted(enumerate(ent_array), key=lambda x: x[1], reverse=True)]) + '\n')
