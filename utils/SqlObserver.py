...
class Source(Base):
    ...
    def get_or_create(cls, filename, md5sum, session):
        ...
        assert md5sum_ == md5sum, "Weird: found md5 mismatch for {}: {} != {}".format(filename, md5sum, md5sum_)
...
class Experiment(Base):
    ...
    def get_or_create(cls, ex_info, session):
        ...
        return cls(name=name, dependencies=dependencies, sources=sources, md5sum=md5, base_dir=ex_info["base_dir"])

    experiment_id = sa.Column(sa.Integer, primary_key=True)
    name = sa.Column(sa.String(32))
    md5sum = sa.Column(sa.String(32))
    base_dir = sa.Column(sa.String(64))
    sources = sa.orm.relationship("Source", secondary=experiment_source_association, backref="experiments")
    dependencies = sa.orm.relationship("Dependency", secondary=experiment_dependency_association, backref="experiments")
...
class Run(Base):
    ...
    status = sa.Column(sa.Enum("RUNNING", "COMPLETED", "INTERRUPTED", "TIMEOUT", "FAILED", "QUEUED", "MANUAL", name="statuses"))
    ...
    # artifacts = backref
    resources = sa.orm.relationship("Resource", secondary=run_resource_association, backref="runs")
        ...
        for name, d in metrics_by_name.items():
            ...
            with self.lock:
                for step, val, timestamp in zip(d["steps"], d["values"], d["timestamps"]):
                    m = Metric(run_id=self.run.run_id, name=name, step=step, value=val, timestamp=timestamp)
                    self.session.add(m)
                    # self.save(add=m)
                self.session.commit()
